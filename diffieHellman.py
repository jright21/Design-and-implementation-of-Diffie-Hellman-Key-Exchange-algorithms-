# diffie_hellman.py
import secrets
from typing import Tuple

class DiffieHellman:
    # 2048-bit MODP Group (Group 14)
    P = int('''
    FFFFFFFF FFFFFFFF C90FDAA2 2168C234 C4C6628B 80DC1CD1
    29024E08 8A67CC74 020BBEA6 3B139B22 514A0879 8E3404DD
    EF9519B3 CD3A431B 302B0A6D F25F1437 4FE1356D 6D51C245
    E485B576 625E7EC6 F44C42E9 A637ED6B 0BFF5CB6 F406B7ED
    EE386BFB 5A899FA5 AE9F2411 7C4B1FE6 49286651 ECE45B3D
    C2007CB8 A163BF05 98DA4836 1C55D39A 69163FA8 FD24CF5F
    83655D23 DCA3AD96 1C62F356 208552BB 9ED52907 7096966D
    670C354E 4ABC9804 F1746C08 CA18217C 32905E46 2E36CE3B
    E39E772C 180E8603 9B2783A2 EC07A28F B5C55DF0 6F4C52C9
    DE2BCBF6 95581718 3995497C EA956AE5 15D22618 98FA0510
    15728E5A 8AACAA68 FFFFFFFF FFFFFFFF
    '''.replace(' ', '').replace('\n', ''), 16)
    
    G = 2  # Generator

    def __init__(self):
        self._private_key = self._generate_private_key()
        self._public_key = self._generate_public_key()
        self.shared_secret = None

    def _generate_private_key(self) -> int:
        """Generate a secure private key."""
        return secrets.randbelow(self.P - 2) + 1

    def _generate_public_key(self) -> int:
        """Generate public key."""
        return pow(self.G, self._private_key, self.P)

    @property
    def public_key(self) -> int:
        return self._public_key

    def validate_public_key(self, other_public_key: int) -> bool:
        """Validate the other party's public key."""
        if not 2 <= other_public_key <= self.P - 2:
            return False
        # Verify that public_key^q mod p = 1 (where q is (p-1)/2)
        q = (self.P - 1) // 2
        if pow(other_public_key, q, self.P) == 1:
            return False
        return True

    def generate_shared_secret(self, other_public_key: int) -> bytes:
        """Generate shared secret and return it as bytes."""
        if not self.validate_public_key(other_public_key):
            raise ValueError("Invalid public key")
        self.shared_secret = pow(other_public_key, self._private_key, self.P)
        return str(self.shared_secret).encode()
